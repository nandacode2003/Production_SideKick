...
async function seed() {
  ...
  // insertMany bypasses pre-save hook, so we insert already-hashed password directly
  ...
  await Chat.create({
    match: m2._id,
    participants: [byName['Arjun Patel']._id, byName['Vikram Singh']._id],
    messages: [
      { sender: byName['Arjun Patel']._id,  text: 'Hey! Ready for cricket on Saturday?', read: true,  createdAt: new Date() },
      { sender: byName['Vikram Singh']._id, text: 'Absolutely! See you at 6AM 🏏',        read: false, createdAt: new Date() },
    ],
    lastMessage: { text: 'Absolutely! See you at 6AM 🏏', sender: byName['Vikram Singh']._id, createdAt: new Date() },
  });
  await Chat.create({
    match: m3._id,
    participants: [byName['Rahul Kumar']._id, byName['Rohan Mehta']._id],
    messages: [
      { sender: byName['Rahul Kumar']._id, text: 'Fellow traveler! Where next?', read: true, createdAt: new Date() },
    ],
    lastMessage: { text: 'Fellow traveler! Where next?', sender: byName['Rahul Kumar']._id, createdAt: new Date() },
  });

  console.log('✅ Created 3 sample matches + 2 chat rooms');
  console.log('\n🔑 Demo credentials (password: sidekick123):');
